...
from robot_interfaces.srv import MultiTarget2GOAP, SetID, RobotStationCheck,RobotStateUpdate , RobotStationUpdate , RobotStateCheck, RobotLiftCheck
...
class MultiStationNavigationAndDocking(Node):
    def __init__(self):
        ...
        # Call lift controller service
        self.lift_controller_cbg = MutuallyExclusiveCallbackGroup()
        self.lift_controller = self.create_client(SetBool, '/lift_on',callback_group=self.lift_controller_cbg)

        # Crate a service client for checking lift status
        self.robot_lift_check_cbg = MutuallyExclusiveCallbackGroup()
        self.robot_lift_check = self.create_client(RobotLiftCheck, 'check_lift_status', callback_group=self.robot_lift_check_cbg)
        ...
        while not self.lift_controller.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Waiting for /lift_on service...')
        while not self.robot_lift_check.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Waiting for /check_lift_status service...')
    ...
    def multi_station_callback(self, request, response):
        ...
        picked_values = request.picked  # Get the picked values from the request
        ...
        # Check if the picked array length matches the target_names length
        if len(picked_values) != len(target_names):
            self.get_logger().warning(f"Length of picked values ({len(picked_values)}) does not match length of target names ({len(target_names)}). Using defaults where needed.")
        ...
        threading.Thread(target=self.process_stations, args=(target_names, picked_values), daemon=True).start()
        
        return response
    
    def process_stations(self, target_names,picked_values):
        ...
        for i,target in enumerate(target_names):
            ...
            picked = picked_values[i] if i < len(picked_values) else False
            ...
            # Control lift based on picked value
            if not self.control_lift(picked):
                self.get_logger().error(f"Failed to {'pick up' if picked else 'drop off'} at station: {target}")
                continue
    ...
    def control_lift(self, lift_up):
        """
        Call the /lift_on service to control the robot's lift.
        True means lift up (pick), False means lift down (drop).
        """
        request = SetBool.Request()
        request.data = lift_up
        future = self.lift_controller.call_async(request)
        
        while not future.done():
            time.sleep(0.1)
        
        if future.result() is not None:
            self.get_logger().info(f"Lift {'raised' if lift_up else 'lowered'} successfully.")
            return True
        else:
            self.get_logger().error(f"Failed to {'raise' if lift_up else 'lower'} lift.")
            return False
